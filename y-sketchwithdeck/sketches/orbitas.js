export default function sketch(p) {
  const STATE_CONFIG = {
    orbitToSolar: { name: 'orbit → solar', orbits: 8 },
    solarToGalaxy: { name: 'solar → galaxy', orbits: 8 },
    galaxyToLocal: { name: 'galaxy → local', orbits: 8 },
    localToCluster: { name: 'local → cluster', orbits: 8 },
    universe: { name: 'universe', orbits: 8 },
    multiverse: { name: 'multiverse', orbits: 8 },
    unity: { name: 'unity', orbits: 8 },
    singularity: { name: 'singularity', orbits: 8 }
  };
  
  const SINGULARITY_DURATION = 60000;
  ...
  function getTransitionT(stateName) {
    if (['universe', 'multiverse', 'unity', 'singularity'].includes(stateName)) {
      ...
      if (stateName === 'singularity') {
        ...
        let t = elapsed / SINGULARITY_DURATION;
        if (t > 1) t = 1;
        return easeInOut(t);
      }
